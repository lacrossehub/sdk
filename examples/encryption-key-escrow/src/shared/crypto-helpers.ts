 ...
 * Used to encrypt wallet bundles with Turnkey's escrow public key.
 */
export async function encryptWithPublicKey(
  ...
  const result = new Uint8Array(65 + iv.length + ciphertext.byteLength);
 ...
 * Used to decrypt wallet bundles after exporting the decryption key from Turnkey.
 */
 ...
 * Note: In JavaScript, this is best-effort due to GC and JIT optimizations.
 */
export function secureWipe(data: Uint8Array | string): void {
  if (typeof data === "string") {
    // Strings are immutable in JS, we can only hope GC clears them
    return;
  }
